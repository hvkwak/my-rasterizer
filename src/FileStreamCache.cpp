 ...
 */
bool FileStreamCache::init(size_t cap){
  capacity = cap;
  return true;
}
 ...
 */
std::ofstream& FileStreamCache::get(int id, const std::filesystem::path& p) {
 ...
 */
void FileStreamCache::close_all() {
  for (auto &[id, co] : m) {
    co.os.close();
 ...
 */
void FileStreamCache::touch(int id) {
  auto &co = m.at(id);
  lru.erase(co.lru_it);
  lru.push_front(id);
  co.lru_it = lru.begin();
 ...
 */
void FileStreamCache::evict_one() {
  int victim = lru.back();
  lru.pop_back();
